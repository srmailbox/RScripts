...
# Q2a: intercepts only ####
Vars2a = data.frame(matrix(NA,nrow=NSims, ncol=3))
...
for(i in 1:NSims){
  ...
  dat$ranInt = ranInts[dat$subj]
  dat$dv = 1.75^rnorm(nrow(dat), mean=dat$ranInt)
  datInts = by(dat, INDICES = dat$subj, FUN=function(x) mean(x$dv))
  dat.lme = lmer(dv~1+(1|subj), dat)
...
colnames(Vars2a)=c("true", "mod", "obs")
colnames(Cors2a)=c("trueVmod", "trueVobs", "modVobs")

# plot(Vars2a$true, Vars2a$mod-Vars2a$true, col=ifelse(Vars2a$mod-Vars2a$true<0,1,2), cex=.2)
# points(Vars2a$true, Vars2a$obs-Vars2a$true, col=ifelse(Vars2a$obs-Vars2a$true<0,3,4), cex=.2)
summary(Cors2a)

# Q2b: ... what if there's a "main Effect" embedded, no random slope ####

Vars2b = data.frame(matrix(NA,nrow=NSims, ncol=3))
...
for(i in 1:NSims){
  ...
  dat$ranInt = ranInts[dat$subj]
  dat$dv = 1.75^rnorm(nrow(dat), mean=dat$ranInt+.5*as.numeric(dat$cond))
  datInts = by(dat, INDICES = dat$subj, FUN=function(x) mean(x$dv))
  dat.lme = lmer(dv~cond+(1|subj), dat)
...
colnames(Vars2b)=c("true", "mod", "obs")
colnames(Cors2b)=c("trueVmod", "trueVobs", "modVobs")
# plot(Vars2b$true, Vars2b$mod-Vars2b$true, col=ifelse(Vars2b$mod-Vars2b$true<0,1,2), cex=.2)
# points(Vars2b$true, Vars2b$obs-Vars2b$true, col=ifelse(Vars2b$obs-Vars2b$true<0,3,4), cex=.2)
summary(Cors2b)
...
Vars2c = data.frame(matrix(NA,nrow=NSims, ncol=3))
...
colnames(Vars2c)=c("true", "mod", "obs")
colnames(Cors2c)=c("trueVmod", "trueVobs", "modVobs")
# plot(Vars2c$true, Vars2c$mod-Vars2c$true, col=ifelse(Vars2c$mod-Vars2c$true<0,1,2), cex=.2)
# points(Vars2c$true, Vars2c$obs-Vars2c$true, col=ifelse(Vars2c$obs-Vars2c$true<0,3,4), cex=.2)
summary(Cors2c)

# Q2d: ...and what if we include the random "slope" ####

Vars2d = data.frame(matrix(NA,nrow=NSims, ncol=6))
Cors2d = data.frame(matrix(NA,nrow=NSims, ncol=6))
...
for(i in 1:NSims){
  ...
  Cors2d[i,] = c(cor(1.75^ranInts, ranef(dat.lme)$subj$`(Intercept)`)
...
colnames(Vars2d)=c("i.true", "i.mod", "i.obs", "s.true", "s.mod", "s.obs")
colnames(Cors2d)=c("i.trueVmod", "i.trueVobs", "i.modVobs", "s.trueVmod", "s.trueVobs", "s.modVobs")
# plot(Vars2d$true, Vars2d$mod-Vars2d$true, col=ifelse(Vars2d$mod-Vars2d$true<0,1,2), cex=.2)
# points(Vars2d$true, Vars2d$obs-Vars2d$true, col=ifelse(Vars2d$obs-Vars2d$true<0,3,4), cex=.2)
summary(Cors2d)
